...
class ReleaseTransaction:
    """
    Rollback-стек. Каждый шаг регистрирует компенсирующее действие.
    При вызове rollback() действия выполняются в обратном порядке.
    """
...
def step_update_versions(tx: ReleaseTransaction, new_version: str):
    """Обновляет версии в Cargo.toml и package.json, регистрирует rollback."""
    ...
    # Сохраняем оригиналы для rollback
...
def step_git_commit_push(tx: ReleaseTransaction, new_version: str, branch: str):
    """Коммитит изменения версий и пушит. Регистрирует rollback."""
...
# --- Version selection (без изменений) ---
...
def check_github_token(token: str):
    try:
        ...
        gh.get_user().login  # реальный запрос
    except Exception as e:
